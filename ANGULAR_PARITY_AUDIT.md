...
The legacy Angular `datagrid` is intentionally excluded as a target. `datagridex` is the canonical grid baseline.
...
| **To-Do** | `Datagridex` (replace legacy `Datagrid`) | Create canonical React `Datagridex` rather than extending the old `Datagrid`. Match Angular's typed rows/columns/groups, intrinsic/fitted sizing, pinning, menu, multi-sort, filtering, templates/editors, validation, details/detail pane, leading actions, new row, client/manual/virtual paging, selection/group selection, grouping, row reorder, row numbers, toolbar/statusbar/footer, data-context adapter, public methods, events, keyboard/ARIA behavior, and docs. Deprecate or remove the old docs route/export according to release policy. | **P0-04** |
| **To-Do** | Core data and adapters | React has the basic data context/data sources but lacks Angular's detail data source/context manager, form model/bridge, undo manager, navigation cursor, NodeStore, errors, and richer adapters. Port framework-neutral behavior and provide hooks/adapters for Datagridex and FormBuilder. | **P0-05** |
| **To-Do** | Test and accessibility baseline | Add a React component test harness, DOM/interaction tests, keyboard/focus tests, axe checks, and theme/RTL coverage. Start with tokens/providers/Datagridex, then require tests for every parity task. | **P0-06** |
...
| **To-Do** | Rich media: `ImageEditor` | Port the Angular image editor's actual public feature set and docs, using React state/callback conventions and the existing token/icon/provider infrastructure. | **P1-14** |
| **To-Do** | Scheduling: `Scheduler`, `GanttChart` | Scheduler is missing calendars/visibility controls, glyphs, resource grouping, time-scale lines, disabled dates, timeline scale, week-number variants, conflict detection, auto-all-day/unavailable ranges, chrome, event CRUD, and view/date callbacks. Gantt needs chrome plus the Angular click/double-click/move/resize/dependency/slot events and its critical-path/resource/undo/virtual-scroll behavior. | **P1-15** |
...
| **To-Do** | Blocks | React has App Shell, Dashboard, Authentication, Settings, Feeds, Project Workspace, Support Desk, and Email. Add Angular's Charts, Cookie Consent, Operations Grid (using `Datagridex`), REST Client Workflow, and Stocks blocks; then reconcile existing block examples. | **P2-05** |
...
> Implement P0-04 from `ANGULAR_PARITY_AUDIT.md`. Use only Angular `projects/spruce-ui/datagridex` and its Datagridex docs as the grid baseline; do not port or extend the old Angular/React Datagrid API. Create a generic typed React `Datagridex` with idiomatic render props/components for cell templates, editors, details, detail pane, and leading actions; port every applicable input/output/public method, data-context adapter, keyboard/ARIA behavior, CSS/token behavior, docs example, and test. Export all public types. Replace the React docs navigation route with Datagridex and document the legacy Datagrid migration/deprecation. Run build, docs build, lint, interaction tests, and light/dark visual checks.

### P0-05 — core data

> Implement P0-05 from `ANGULAR_PARITY_AUDIT.md`. Compare `../spruce-ng/projects/core-data` with React `src/data`. Port all framework-neutral missing behavior: detail sources/context manager, form model/bridge, undo, cursor, NodeStore, errors, types, and adapters. Expose idiomatic React hooks while preserving non-React data classes. Add unit tests matching Angular scenarios, update docs/exports, and verify Datagridex/FormBuilder integration boundaries.

### P0-06 — tests and accessibility baseline

> Implement P0-06 from `ANGULAR_PARITY_AUDIT.md`. Add the minimal React test stack consistent with this Vite/React 19 repository, shared render helpers with `SpruceProvider`, user interaction utilities, axe checks, keyboard/focus assertions, theme/RTL helpers, and CI scripts. Seed tests for providers, Button, overlays, form validation, and Datagridex. Do not rewrite components except for defects exposed by the tests.
