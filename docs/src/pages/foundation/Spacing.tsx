...
export function SpacingPage() {
  return (
    <FoundationPageShell variant="spacing" title="Spacing" description="Spacing tokens create layout rhythm, breathable controls, and consistent separation between interface layers. The scale is based on a compact system that still leaves room for expressive page structure and generous content sections.">
      ...
      <section id="scale" className="doc-section"><h2>Scale</h2><p className="section-desc">The spacing scale spans hairline adjustments through full-page section gaps. Smaller steps are optimized for controls and dense content, while larger steps support layout rhythm.</p><div className="spacing-list" role="list" aria-label="Spacing scale">{SPACINGS.map(([label, token, value, usage]) => <div key={token} className="spacing-row" role="listitem"><div className="spacing-row__meta"><code>{token}</code><span>{value}</span></div><div className="spacing-row__demo"><div className="demo-container" style={{ gap: `var(${token})` }}><div className="demo-box" /><div className="demo-box" /></div><span className="demo-label">{label}</span></div><p>{usage}</p></div>)}</div></section>

      <section id="layout-rhythm" className="doc-section"><h2>Layout Rhythm</h2><p className="section-desc">Different ranges in the spacing scale serve different layout jobs. Reuse the same ranges consistently so forms, panels, tables, and docs pages feel related.</p><div className="rhythm-grid">{RHYTHM.map(([title, range, body]) => <article key={title} className="rhythm-card"><div className="rhythm-card__badge">{range}</div><h3>{title}</h3><p>{body}</p></article>)}</div></section>

      <section id="radius" className="doc-section"><h2>Border Radius</h2><p className="section-desc">Radius tokens soften containers and controls without disconnecting them from the product’s enterprise tone. Keep radius choices limited and tied to component roles.</p><div className="radius-grid">{RADII.map(([label, token, value, usage]) => <article key={token} className="radius-card"><div className="radius-card__preview" style={{ borderRadius: `var(${token})` }}><span>Preview</span></div><div className="radius-card__body"><div className="radius-card__row"><strong>{label}</strong><span>{value}</span></div><code>{token}</code><p>{usage}</p></div></article>)}</div></section>
