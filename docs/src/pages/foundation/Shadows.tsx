...
export function ShadowsPage() {
  return (
    <>
      ...
      <section id="shadow-levels" className="doc-section"><h2>Shadow Levels</h2><p className="section-desc">Each shadow token represents a consistent elevation step. Keep the progression narrow so users can still understand which surface is actually on top.</p><table className="token-table" aria-label="Shadow levels"><thead><tr><th>Token</th><th>Value</th><th>Usage</th></tr></thead><tbody>{SHADOWS.map(([label, token, value, usage]) => <tr key={token}><td><strong>{label}</strong> <code>{token}</code></td><td><code>{value}</code></td><td>{usage}</td></tr>)}</tbody></table></section>

      <section id="layering" className="doc-section"><h2>Layering Guidance</h2><p className="section-desc">Shadows and stacking should reinforce each other. A modal should not only sit above page content numerically, it should also feel visually elevated relative to the layers beneath it.</p><div className="principles-grid">{LAYERS.map(([title, range, body]) => <article key={title} className="principle-card"><code>{range}</code><h3>{title}</h3><p>{body}</p></article>)}</div></section>
