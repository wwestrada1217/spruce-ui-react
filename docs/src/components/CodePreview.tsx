...
/**
 * Quoted strings, matched on the *escaped* source.
 */
const STRING_RE =
  /&quot;(?:(?!&quot;)[\s\S])*?&quot;|&#039;(?:(?!&#039;)[\s\S])*?&#039;|`(?:[^`\\]|\\.)*`/g;

/**
 * Numbers, excluding the digits inside an HTML entity.
 */
  ...
  return (
    <div className={`cp${compact ? ' cp--compact' : ''}`}>
        ...
        <div className="cp__actions">
          ...
          {propertyPanelAvailable && (
            <button
              ...
              aria-label={propertyPanelOpen ? 'Hide properties' : 'Show properties'}
            >
      ...
      {currentTab === 'preview' && (
        ...
        >
          <div
            className={`cp__preview${propertyPanelVisible ? ' cp__preview--split' : ''}`}
            role="tabpanel"
            aria-label="Preview"
          >
            {children}
      ...
      {/* Code panel */}
      {currentTab === 'code' && (
        <div className="cp__code" role="tabpanel" aria-label="Code">
            ...
            <div className="cp__file-tabs" role="tablist" aria-label="Code files">
              {resolvedFiles.map((file, i) => (
                <button
                  ...
                  tabIndex={activeFileIndex === i ? 0 : -1}
